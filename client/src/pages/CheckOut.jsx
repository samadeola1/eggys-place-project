...
const CheckOut = () => {
  ...
  const handlePayNow = async () => {
    ...
    if (!recipientInfo)
      return alert("Please fill in the recipient information.");
      ...
      onSuccess: async (transaction) => {
        try {
          const response = await fetch(
            "https://eggys-place-project-4xj0.onrender.com/api/order",
            {
              method: "POST",
              headers: {
    ...
    <>
      <main className="wrapper md:grid grid-cols-3 py-1 bg-[#2F2F2F] gap-6 ">
        ...
        <section className="col-span-2 bg-[#100101] mt-3 p-4 flex flex-col justify-between gap-y-[14px] rounded-lg ">
          {/*div-1  */}
          <Recipient
            handleFormActivity={handleFormActivity}
          ...
          {/*div-2  */}
          <Delivery
            handleFormActivity2={handleFormActivity2}
